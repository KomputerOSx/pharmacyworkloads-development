...
//     });
//
//     // Reset input when the dialog opens or the item changes
//     useEffect(() => {
//         if (open) {
...
//             });
//         }
//     }, [open, itemName]); // Depend on itemName in case the target changes while open (unlikely but safe)
//
//     const isConfirmationMatching = useMemo(() => {
//         if (!itemName) return false; // Handle case where itemName might be empty initially
//         return (
...
//         // No need to reset input here, useEffect handles it on next open
//     };
//
//     const handleConfirm = () => {
//         if (isConfirmationMatching && !isPending) {
//             onConfirm(); // Call the parent's confirmation logic
//         }
//     };
//
//     // Prevent rendering if itemName is not available yet (avoids flicker)
//     if (!itemName && open) {
//         return null; // Or a loading state specific to the dialog if preferred
//     }
//
//     return (
...
//                         disabled={isPending} // Disable input while deleting
...
//                             (!isConfirmationMatching || isPending) &&
