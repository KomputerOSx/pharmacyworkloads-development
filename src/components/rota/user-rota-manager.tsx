...
export function UserRotaManager({
    ...
    //     isPending: isDeletingAssignments,
    // } = useDeleteWeekAssignments();
    // const { mutate: deleteStatusMutate, isPending: isDeletingStatus } = useDeleteWeekStatus(); // Likely not needed
    ...
    const performSave = useCallback(
        (callback?: () => void, markAsDraft = false) => {
            ...
            // Collect only assignments for the current week/team from the *local state*
            Object.entries(assignments).forEach(([key, value]) => {
                if (
                    key.startsWith(currentWeekPrefix) &&
                    key.endsWith(`-${teamId}`)
            ...
            // Ensure IDs are unique and basic fields are present
            const uniqueValidAssignments = Array.from(
                new Map(gatheredAssignments.map((a) => [a.id, a])).values(),
            ).filter(
                (a) =>
            ...
            // --- Save Assignments ---
            // The backend for saveAssignments needs to handle the case where uniqueValidAssignments is empty.
            // If empty, it should delete all existing assignments for that weekId/teamId.
            saveAssignments(
                {
                    weekId,
                    teamId,
                    assignmentsToSave: uniqueValidAssignments, // Can be empty if cleared
                    userId: currentUserId,
                },
                {
                    onSuccess: () => {
                        ...
                        if (
                            ...
                            // If saving changes to published, ensure status stays published
                            // (or re-assert it if necessary, backend might handle this)
                            setStatusMutate({
                                weekId,
                                teamId,
                                orgId,
                        ...
                        if (callback) callback(); // Execute callback on success
                    },
                    onError: (error) => {
                        // Keep hasUnsavedChanges true on error, let user retry
    ...
    const handleSaveDraft = useCallback(() => {
        performSave(() => {
            toast.info("Draft saved.");
        }, true); // Mark as draft = true
    }, [performSave]);

    const handleSaveChanges = useCallback(() => {
        performSave(() => {
            toast.success("Changes saved to published rota.");
        }, false); // Mark as draft = false
    }, [performSave]);
    ...
    useHotkeys(
        ...
        (event) => {
            ...
            if (isSavingAssignments || isSettingStatus) {
                toast.warning("Already saving...");
                return;
            }

            if (currentStatus === "published") {
                console.log("Ctrl+S: Saving changes to published");
                handleSaveChanges();
            } else {
                console.log("Ctrl+S: Saving draft");
                handleSaveDraft();
            }
        },
        [
    ...
    // --- Status Handling (No changes needed in logic itself) ---
    const handleSetStatus = useCallback(
        (newStatus: "draft" | "published") => {
            const actionToConfirm = () => {
                if (newStatus === "published") {
                    // Save first, then set status and trigger notification dialog
                    performSave(() => {
                        setStatusMutate(
                            {
                                weekId,
                    ...
                    }, false); // performSave with markAsDraft = false
                } else {
                    // Set status directly (saving draft implicitly happens via performSave if needed)
                    setStatusMutate(
                        {
                            weekId,
                            teamId,
                        ...
                        }, // Reset changes only if setting to draft succeeds
                    );
                }
            };
    ...
    // --- Non-Destructive Clear Rota ---
    const handleClearRota = useCallback(() => {
        showConfirmation(
            "Clear Rota View",
            "This will clear the rota locally for this week. Save changes to remove all assignments permanently.",
            () => {
                // Clear only the assignments for the current week from the local state
                setAssignments((prev) => {
                    const newState = { ...prev };
                    const prefix = `${weekId}-`;
                    const suffix = `-${teamId}`;
                ...
                setHasUnsavedChanges(true);
                // Optionally, could locally set status display to 'Draft' if needed,
                // but saving will handle the actual status update.
                toast.info(
                    "Rota cleared locally. Save changes to make it permanent.",
                );
            },
        );
    }, [weekId, teamId, showConfirmation]);
    // --- End Clear Rota ---
    ...
    const handleExport = useCallback(
        async (
            ...
            try {
                ...
                if (format === "csv") exportToCSV(...commonArgs);
                else if (format === "excel") await exportToExcel(...commonArgs);
                else if (format === "pdf") await exportToPDF(...commonArgs);
    ...
    // --- Context Menu Handlers (No changes needed) ---
    const handleContextMenu = useCallback(
        (
            e: React.MouseEvent,
            userId: string,
    ...
    // --- Copy From Previous Week (No changes needed) ---
    const copyFromPreviousWeek = useCallback(async () => {
        const previousWeekDate = subWeeks(date, 1);
        const previousWeekNumber = getISOWeek(previousWeekDate);
        const previousWeekYear = format(previousWeekDate, "yyyy");
            ...
            async () => {
                ...
                try {
                    ...
                    const newAssignmentsState: Record<
                        ...
                        StoredAssignment[]
                    > = {};
                    const newActiveUserIds = new Set<string>(activeUserIds);
                    ...
                    previousWeekAssignments.forEach((prevAss) => {
                        ...
                        if (!newAssignmentsState[cellKey])
                            newAssignmentsState[cellKey] = [];
                        newAssignmentsState[cellKey].push(newAssignment);
                        newActiveUserIds.add(newAssignment.userId);
                    });

                    setAssignments(newAssignmentsState);
                    setActiveUserIds(Array.from(newActiveUserIds));
                    setHasUnsavedChanges(true);
                    toast.success(
                        `Copied ${previousWeekAssignments.length} assignments from Week ${previousWeekNumber}. Remember to save.`,
                    );
                    ...
                    // Consider if reverting changes is desired on error
                    // setHasUnsavedChanges(false);
                } finally {
                    setIsLoadingCopy(false);
                }
    ...
    return (
        <div className="space-y-4">
                        ...
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Button
                                    ...
                                    onClick={handleClearRota}
                                    className="flex items-center gap-1"
                                    // No disabled state based on deletion needed anymore
                                >
                                    <Trash2 className="h-4 w-4" />
                                    <span className="hidden sm:inline">
                                        Clear Rota View
            ...
            <TooltipProvider>
                <Tooltip>
                    <TooltipTrigger className="hidden sm:inline-block cursor-default p-1 rounded-md text-muted-foreground hover:bg-muted">
                        <span className="text-xs">Save: </span>
                        <Kbd>Ctrl</Kbd>+<Kbd>S</Kbd>
                    </TooltipTrigger>
                    <TooltipContent>
                        Use Ctrl+S or ⌘+S to save changes.
                    </TooltipContent>
                </Tooltip>
            </TooltipProvider>
