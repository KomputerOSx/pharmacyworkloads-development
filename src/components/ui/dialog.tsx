...
>(({ className, children, ...props }, ref) => (
    <DialogPortal>
        <DialogOverlay className="fixed inset-0 z-50 bg-black/80 overflow-auto flex items-center justify-center p-4">
            <DialogPrimitive.Content
                ref={ref}
                className={cn(
                    "relative w-full max-w-lg rounded-lg border bg-background p-6 shadow-lg " +
                        "transform transition-all duration-300 " +
                        "my-auto " + // Ensures vertical centering with sufficient margin
                        "max-h-[calc(100vh-2rem)] " + // Prevents overflow on small screens
